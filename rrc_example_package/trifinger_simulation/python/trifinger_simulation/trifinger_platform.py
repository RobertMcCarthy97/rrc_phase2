...
class TriFingerPlatform:
    ...
    def __init__(
        ...
        if config_dir is not None:
            self.tricamera = camera.create_trifinger_camera_array_from_config(
                config_dir, calib_filename_pattern=calib_filename_pattern,
                pybullet_client_id=self.simfinger._pybullet_client_id
                )
        else:
    ...
    def get_dice_states(self):
        dice_pos, dice_orient = [], []
        dice_lin_vel, dice_ang_vel = [], []
        for die in self.dice:
            # positions
            pos, orient = die.get_state()
            dice_pos.append(pos), dice_orient.append(orient)
            # velocities
            lin_vel, ang_vel = die.get_velocity()
            dice_lin_vel.append(lin_vel), dice_ang_vel.append(ang_vel)
        return dice_pos, dice_orient, dice_lin_vel, dice_ang_vel
