...
class RandomizedEnvWrapper(gym.Wrapper):
    def __init__(self, env, camera_fps=10.0, visualize=False, flatten_obs=False, time_step_s=0.001, random_scale=1.0, task='move_cube'):
        super().__init__(env)
        self.first_run = True
        self.randomizer = TriFingerRandomizer(timestep_low=0.95*time_step_s, timestep_high=1.2*time_step_s, random_scale=random_scale, task=task)
    ...
    def reset(self, difficulty=None, init_state='normal', noisy=False, noise_level=1):
        ...
        self.env.set_cube_scale(cube_width_scale)
        ...
        if self.first_run:
            ...
            self.cube_ids = []
            if self.task == 'move_cube':
                self.cube_ids.append(self.env.platform.cube._object_id)
            else:
                for die in self.env.platform.dice:
                    self.cube_ids.append(die._object_id) # are these tracked properly over multiple resets?
    ...
    def sample_noisy_cube(self, obs):
        if self.task == 'move_cube':
            ...
            pos, ori = [], []
            for i, _ in enumerate(obs['object_observation']['position']):
                noise = self.randomizer.sample_cube_noise()
                q_obj = R.from_quat(obs['object_observation']['orientation'][i])
                q_noise = R.from_euler('ZYX', noise['cube_ori'], degrees=False)
                # append to obs
                pos.append(obs['object_observation']['position'][i] + noise['cube_pos']) # list()?
                ori.append((q_obj * q_noise).as_quat())
    ...
    def randomize_param(self):
        ...
        for cube_id in self.cube_ids:
            p.changeDynamics(bodyUniqueId=cube_id, linkIndex=-1,
                             physicsClientId=self.client_id,
